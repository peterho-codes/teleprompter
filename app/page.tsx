    ...
    <div className="app-root">
      ...
      <div className="island-wrapper">
        <div className={`island ${mode === "present" ? "island--active" : ""}`}>
          <div className="island-left">
            ...
            />
            {(isLive || isPaused) && !isAndroid && <Waveform active={isLive} color="#c084fc" />}
      ...
      {speechError && (
        <div className="error-banner" role="alert">
          ...
          <button className="error-dismiss" onClick={() => setSpeechError(null)}>✕</button>
        </div>
      )}
      ...
      <style>{`
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .error-banner { display: flex; align-items: center; justify-content: space-between; gap: 12px; background: rgba(239,68,68,0.12); border: 1px solid rgba(239,68,68,0.3); border-radius: 12px; padding: 10px 16px; max-width: 640px; width: 90%; font-size: 13px; color: #fca5a5; line-height: 1.4; }
        .error-dismiss { background: none; border: none; color: #f87171; cursor: pointer; font-size: 14px; padding: 2px 4px; flex-shrink: 0; }
      `}</style>
    </div>
  );
}
