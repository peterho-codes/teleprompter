...
export function useSpeechRecognition(options: SpeechRecognitionOptions = {}) {
  ...
  const createAndStart = useCallback(() => {
    ...
    rec.onend = () => {
      ...
      // 250ms gives the hardware audio stack time to release before re-acquiring.
      // Guard: only restart if still in listening state and not already restarting.
      if (statusRef.current === "listening") {
        clearRestartTimer();
        restartTimerRef.current = setTimeout(() => {
          if (statusRef.current === "listening" && !isStartingRef.current) {
            createAndStart();
          }
        }, 250);
      }
